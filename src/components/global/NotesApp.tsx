...
type Section = "menu" | "education" | "experience" | "certificates" | "skills";
// | 'roles'
// | 'activities'
// | 'competitions';
...
const NotesApp = ({ isOpen, onClose }: NotesAppProps) => {
  ...
  const [activeImageIndices, setActiveImageIndices] =
    useState<ImageIndicesState>({});
  ...
  const handlePrevImage = (itemId: string, images: readonly Image[]) => {
    setActiveImageIndices((prevIndices) => ({
      ...prevIndices,
      [itemId]:
        ((prevIndices[itemId] ?? 0) - 1 + images.length) % images.length,
    }));
  };
  ...
  const certificates = userConfig.courses || [];
  ...
  const renderCourses = () => (
    <div className="space-y-6">
      {renderBackButton()}
      <h2 className="text-2xl font-bold text-gray-200 mb-6">Certificates</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-gray-400">
            Explore my professional work experience
          </p>
        </div>
        ...
        >
          ...
          <p className="text-gray-400">
            Check out the certificates I have earned
          </p>
        </div>
    ...
    >
      <div className="flex flex-col flex-grow min-h-0 h-full">
        <div className="overflow-y-auto flex-grow min-h-0 p-4 md:p-6">
          ...
          {activeSection === "certificates" && renderCourses()}
